import com.android.build.api.artifact.ArtifactTransformationRequest // JW
import com.android.build.api.artifact.SingleArtifact // JW
...
val canonicalVersionName = "8.19.2-3.0-JW"
val currentHotfixVersion = 0
val maxHotfixVersions = 100
val mollyRevision = 3

// JW: added
val abiCodes: Map<String, Int> = mapOf("armeabi-v7a" to 1, "arm64-v8a" to 2, "x86" to 2, "x86" to 3, "x86_64" to 4)
...
android {
  ...
  defaultConfig {
    ...
    versionName = sourceVersionNameWithRevision // JW
    ...
    buildConfigField("long", "BUILD_TIMESTAMP_OR_ZERO", "1000L") // JW: fixed time for reproducible builds, is not used anyway
...
abstract class RenameApkTask : DefaultTask() {
  ...
  fun rename() {
    transformationRequest.get().submit(this) { artifact ->
      ...
      val newName2 = if (!versionName.isNullOrEmpty()) {
        newName.replace("app", "MollyIm-Android")
      } else {
        newName
      }
      val newFile = File(outFolder.get().asFile, newName2)
