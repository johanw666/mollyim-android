...
import org.thoughtcrime.securesms.groups.GroupChangeException; // JW
import org.thoughtcrime.securesms.groups.GroupDoesNotExistException;
import org.thoughtcrime.securesms.groups.GroupId;
import org.thoughtcrime.securesms.groups.GroupManager; // JW
...
public final class GroupsV2StateProcessor {
  ...
  static class ProfileAndMessageHelper {
    ...
    void storeMessage(@NonNull DecryptedGroupV2Context decryptedGroupV2Context, long timestamp) {
      ...
      } else if (!TextSecurePreferences.blockedContactsCantAddYouToGroups(context) || !Recipient.resolved(RecipientId.from(editor.get(), null)).isBlocked()) { // JW: don't store messages from blocked contacts
