...
import static org.thoughtcrime.securesms.database.MmsTable.DATE_RECEIVED;
import static org.thoughtcrime.securesms.database.MmsTable.DATE_SENT;
import static org.thoughtcrime.securesms.database.MmsTable.TYPE;
import static org.thoughtcrime.securesms.database.MmsTable.MMS_MESSAGE_TYPE;
import static org.thoughtcrime.securesms.database.MmsTable.MMS_STATUS;
import static org.thoughtcrime.securesms.database.MmsTable.TABLE_NAME;
import static org.thoughtcrime.securesms.database.MmsTable.VIEW_ONCE;
...
import static org.thoughtcrime.securesms.database.MmsSmsColumns.SMS_SUBSCRIPTION_ID;
...
public class WhatsappBackupImporter {
            ...
            throws NoExternalStorageException, IOException
    {
        ...
        SmsTable smsDb                   = SignalDatabase.sms();
        ...
        try {
            ...
            while ((item = backup.getNext()) != null) {
                ...
                if (isMms(item)) {
                    ...
                    if (avoidDuplicates && wasMsgAlreadyImported(smsDbTransaction, MmsTable.TABLE_NAME, MmsTable.DATE_SENT, threadId, recipient, item)) continue;
                    ...
                    if (avoidDuplicates && wasMsgAlreadyImported(smsDbTransaction, SmsTable.TABLE_NAME, SmsTable.DATE_SENT, threadId, recipient, item)) continue;
    ...
    private static void insertMms(MmsTable mmsDb, AttachmentTable attachmentDb, WhatsappBackup.WhatsappBackupItem item, Recipient recipient, long threadId, List<Attachment> attachments) throws MmsException {
        ...
        contentValues.put(MMS_STATUS, MmsTable.Status.DOWNLOAD_INITIALIZED);
    ...
    private static void insertSms(SmsTable smsDb, SQLiteDatabase transaction, WhatsappBackup.WhatsappBackupItem item, Recipient recipient, long threadId) {
        SQLiteStatement statement  = PlaintextBackupImporter.createSmsInsertStatement(transaction);
    ...
    private static RecipientId getGroupId(GroupTable groups, WhatsappBackup.WhatsappBackupItem item, Recipient recipient) {
        ...
        List<GroupTable.GroupRecord> groupRecords = groups.getGroupsContainingMember(recipient.getId(), false);
        for (GroupTable.GroupRecord group : groupRecords) {
    ...
    private static void addTranslatedTypeToStatement(SQLiteStatement statement, int index, int type) {
        statement.bindLong(index, SmsTable.Types.translateFromSystemBaseType(type));
    }
    ...
    private static boolean isAppropriateTypeForImport(long theirType) {
        long ourType = SmsTable.Types.translateFromSystemBaseType(theirType);
