...
import kotlin.time.Duration.Companion.minutes // JW: Molly
import kotlin.time.Duration.Companion.seconds // JW: Molly
...
object AccountDataArchiveProcessor {
  ...
  fun export(db: SignalDatabase, signalStore: SignalStore, exportState: ExportState, emitter: BackupFrameEmitter) {
    ...
    val screenLockTimeoutSeconds = TextSecurePreferences.getPassphraseLockTimeout(context) // JW: Molly
  ...
  private fun importSettings(context: Context, settings: AccountData.AccountSettings, importState: ImportState) {
    ...
    if (screenLockTimeoutMinutes != null) {
      TextSecurePreferences.setPassphraseLockTimeout(context, screenLockTimeoutMinutes.minutes.inWholeSeconds) // JW: Molly
    }
